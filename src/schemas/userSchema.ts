...
export const registerSchema = z
  .object({
    ...
    email: z.string(objError).email('Invalid email').openapi({
      example: 'sarat@gmail.com',
      description: 'email',
    }),
    role: userRoles,
    address: z.string(objError).optional().openapi({
...
export const registerCustomerSchema = registerSchema
  .extend({
    role: z
      .string()
...
export const registerEmployeeSchema = registerSchema
  .extend({
    role: z
      .string()
      .transform((val) => (val.trim() === '' ? undefined : val))
...
export const userResponseSchema = z
  .object({
    username: z.string(objError).toLowerCase().nonempty().openapi({
      example: 'sarat123',
      description: 'a unique nonempty username',
    }),
    role: userRoles,
...
export const logoutSchema = z
  .object({
    refreshToken: z.string(objError).nonempty().openapi({
