...
export const adoption = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        logger.error('Failed to save adoption', { message: err.message, stack: err.stack });
        const errors = handleError(err);
...
export const getMyPets = async (req: Request, res: Response): Promise<void> => {
    try{ //! test
        ...
        logger.error('Failed to get user pets', { message: err.message, stack: err.stack });
        const errors = handleError(err);
        res.status(404).json( errors );
    }
}
...
export const getRemains = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        logger.error('Failed to get remaining payment info', { message: err.message, stack: err.stack });
        const errors = handleError(err);
...
export const payment = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        logger.error('Payment processing failed', { message: err.message, stack: err.stack });
        const errors = handleError(err);
        res.status(400).json( errors );
    }
} 
...
export const cancelPets = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        logger.debug(`User ${user_id} requested to cancel pets: ${JSON.stringify(pets)}`);
        const adopt = await cancelingPets(user_id, pets);

        logger.info(`Pets cancelled for user ${user_id}`);
        res.status(200).json( adopt );
    }catch(err: any){
        logger.error('Failed to cancel pets', { message: err.message, stack: err.stack });
        const errors = handleError(err);
        res.status(400).json( errors );
    }
} 
...
export const getAdoption = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        logger.error('Failed to get adoption', { message: err.message, stack: err.stack });
        const errors = handleError(err);
        res.status(400).json( errors );
    }
}