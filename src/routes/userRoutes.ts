...
router.post('/user/employee', validate(registerSchema), authenticate, authorizeRoles('admin'), registerEmployee);
router.get('/user',authenticate ,getUsers); 
router.get('/user/id/:id',authenticate,authorizeRoles('admin'),  getUser); 
router.get('/user/username/:username',authenticate, getUser); 
router.patch('/user/alter', validate(updateUserSchema), authenticate, updated); 
router.patch('/user/role/:username', validate(updateUserSchema), authenticate, authorizeRoles('admin'), updated); 

router.delete('/user/unactive', authenticate, deleteUser); 

router.delete('/user/delete/id/:id', authenticate, authorizeRoles('admin'), deleteUser);
router.delete('/user/delete/username/:username', authenticate, authorizeRoles('admin'), deleteUser);
